...
function singaporeParts(now = new Date()) {
...
function isVyshnaviBirthday(now) {
  const sg = singaporeParts(now);
  return sg.month === VYSHNAVI.birthMonth + 1 && sg.day === VYSHNAVI.birthDay;
}
...
function buildContext(weather, now, birthSky) {
  ...
  const sg = singaporeParts(now);
  const day = DAYS.indexOf(sg.weekday);
  const dateKey = sg.year + '-' +
    String(sg.month).padStart(2, '0') + '-' +
    String(sg.day).padStart(2, '0');
  ...
  return {
    ...
    dayOfMonth: sg.day,
    month: sg.month - 1,
    monthName: MONTHS[sg.month - 1],
    seed: hashString(seedString),
    weather: weather.current,
    sky,
    birthSky,
    isBirthday: birthday,
    age: birthday ? sg.year - VYSHNAVI.birthYear : null
  };
}
