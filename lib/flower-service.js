...
const { buildRecraftPrompt, pickVariation } = require('./prompt');
...
function previewCaption(caption, variation) {
  return {
    ...caption,
    main: caption.main,
    influence: 'PREVIEW · ' + variation.label + ' · ' + variation.region + ' · ' + caption.influence,
    isPreview: true,
...
async function getFlower({ force = false, preview = false } = {}) {
  ...
  const prompt = buildRecraftPrompt(ctx, { variation });
  ...
  const payload = {
    imageUrl,
    caption: preview ? previewCaption(caption, variation) : caption,
    prompt,
    model: 'recraftv4',
    dateKey,
    seed: ctx.seed,
    ...
    region: variation ? variation.region : null,
    preview: !!preview
  };
